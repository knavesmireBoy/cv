...
(function (slice, query) {
  ...
  function validate(arg, m = "toUpperCase") {
    return arg[m]() !== arg ? arg : null;
  }

  function reAssign(arr, i) {
    let res;
    if ((res = arr.shift())) {
      return [res, i];
    }
    return reAssign(arr, i + 1);
  }

  function soAssign(arr, i) {
    let next;
  ...
  function driver(end, van, places, i) {
    if (!van.length) return;
    let next = places[i], j;
    if (end) {
      //inward
      let [tmp, j] = reAssign(van, 1);
      if (tmp) {
        places[i] = tmp;
      }
      return driver(end, van, places, i - j);
    } else if (!places[i + 1]) {
      //arrival
      next = validate(next);
      if (next) {
      ...
      //outward
      next = validate(next) ? next : null;
      van.unshift(next);
      return driver(end, van, places, i + 1);
    }
  }
  ...
  var main = document.querySelector("main"),
    ...
    queryHash = defer(curry3(lazyVal)("map")([getHIMhref, getHash]))(
      curry2(invoke)("hash")
    ),
    ...
    doReload = compose(
      invoke,
      deferPTL(best, compose(ptL(invoke, equals), queryHash), [reEntry, dummy])
    ),
    ...
    listen = function (tgt) {
      ...
      best(deferPTL(equals, getCurrent(), getID(tgt)), [
        compose(resetPicHref, reSetPic),
        enter,
      ])();
      deferScroll();
    },
    ...
    listenBridge = function (e) {
      var cb = curry2(invoke)(e),
        enter = defer(
          lazyEach([compose(listen, defer(getTarget)(e)), prevent])
        )(cb);
    ...
    removeAttr = deferPTL(
      ...
      links,
      "forEach",
      curry3(invokeProp)("href")("removeAttribute")
    ),
    removeListener = deferPTL(invokeProp, main, "removeEventListener", [
      "click",
      listenBridge,
    ]),
    undo = defer(each)([removeAttr, removeListener]),
    getPredicate = compose(
      curry2(getPropBridge)("matches"),
      defer(curry3(invokeProp)(query)("matchMedia"))(window)
    ),
    setup = function () {
      var a = ["dn", "frwl", "yolt", "OHMSS", "daf", "LALD", "FYEO", "tld"],
        start = 1,
        t = "tld",
        b = [t],
        i = a.findIndex((item) => item === t);
      a[i] = null;
      /*
      a.reverse();
      start = 5;
      ...
      */
      ...
      var doReverse = ptL(lazyVal, null, [loader, undo]),
        strategy = compose(
          ...
          doReverse,
          invoke,
          deferPTL(best, getPredicate, [always("slice"), always("reverse")])
        )(),
        doStrategy = compose(
          invoke,
          deferPTL(best, getPredicate, [strategy, dummy])
        ),
        handler = function () {
          if (!getPredicate()) {
            strategy();
